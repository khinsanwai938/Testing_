...
class MAVLinkDroneDriver:
    ...
    def _production_hover_watchdog(self):
        ...
        while self._hover_heartbeat_active:
            ...
            if batt is not None:
                ...
                # 4. FULLY NOMINAL STATE
                else:
                    # Only clear everything back to False if the battery readings are completely nominal
                    has_warned_low = False
                    has_warned_anomaly = False
    ...
    def reset_hover_timeout(self):
        ...
        # 1. Acquire the lock briefly just to check the state safely
        with self._state_lock:
            if self._state == self._STATE_READY:
                should_reset = True

        # 2. Execute the timer reset OUTSIDE the lock to prevent a deadlock
        if should_reset:
    ...
    def change_altitude_absolute(self, target_altitude_meters: float) -> bool:
        ...
        """
        print(f"[DRIVER] Sending local position target frame for alt: {target_altitude_meters:.1f} m")

        # In MAVLink NED coordinates, Z is negative for altitude upwards!
        # -target_altitude_meters sets the height relative to home/takeoff point
        target_z = -float(target_altitude_meters)
        ...
        msg = self.master.mav.set_position_target_local_ned_encode(
            ...
            0x0FF8,                      # type_mask: Enforce only Z position tracking
            0.0, 0.0, target_z,          # x, y, z positions (meters)
            0.0, 0.0, 0.0,               # vx, vy, vz velocities
            0.0, 0.0, 0.0,               # afx, afy, afz accelerations
        ...
        try:
            ...
            print(f"[DRIVER ERROR] Failed to send position target: {e}")
            return False

        # Keep your verified blocking tracking loop below
        TAKEOFF_TIMEOUT = 30
        ...
        while time.time() < deadline:
            ...
            if alt >= (target_altitude_meters - self.ALTITUDE_TOLERANCE):
                print("[DRIVER] Target cumulative altitude reached safely.")
                return True
            ...
            if abs(alt - last_alt) > STUCK_THRESHOLD:
                ...
                print(f"[DRIVER] Climb path stuck at {alt:.2f} m — breaking lock.")
                return False

            time.sleep(1)

        print(f"[DRIVER] Altitude adjustment timed out after {TAKEOFF_TIMEOUT} s.")
        return False
    ...
    def get_battery(self) -> Optional[dict]:
        ...
        if msg:
            ...
            # FIX 1: Removed direct print statement to prevent background thread spam
