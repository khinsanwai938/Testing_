...
class MAVLinkDroneDriver(MAVLinkBaseDriver):
    ...
    def send_body_translation(self, vx: float, vy: float, vz: float):
        ...
        Uses MAV_FRAME_BODY_OFFSET_NED to ensure straight translations without turning.
        """
        # We explicitly DO NOT ignore YAW_RATE. We want to actively force yaw rate to 0.0.
        type_mask = (
            ...
            # | mavutil.mavlink.POSITION_TARGET_TYPEMASK_YAW_IGNORE  <-- Un-ignored
            # | mavutil.mavlink.POSITION_TARGET_TYPEMASK_YAW_RATE_IGNORE <-- Un-ignored
        )
        ...
        self.master.mav.set_position_target_local_ned_send(
            ...
            mavutil.mavlink.MAV_FRAME_BODY_OFFSET_NED, # <-- FIXED: Changed from MAV_FRAME_BODY_NED
            type_mask,
            0, 0, 0,       # Position targets (ignored)
            vx, vy, vz,    # Velocity tracking targets (m/s)
            0, 0, 0,       # Acceleration targets (ignored)
            0,             # Target Yaw (0.0 rad)
            0,             # Target Yaw Rate (0.0 rad/s -> strictly forces NO rotation)
        )
    ...
    def start_mission_if_waypoints_exist(self) -> bool:
        """
        Requests the waypoint count from the flight controller.
        If actual waypoints exist, switches the drone to AUTO mode to begin the mission.
        """
        print(f"{self._LOG_PREFIX} Checking waypoint availability on flight controller...")
        
        # 1. Flush out any stale or leftover mission packets from the buffer
        while self.master.recv_match(type='MISSION_COUNT', blocking=False):
            pass

        # 2. Request the total number of mission items from the autopilot
        self.master.mav.mission_request_list_send(
            self.master.target_system,
            self.master.target_component
        )

        # 3. Wait up to 2 seconds for the flight controller to respond
        msg = self.master.recv_match(type='MISSION_COUNT', blocking=True, timeout=2.0)
        
        if not msg:
            print(f"{self._LOG_PREFIX} Waypoint check failed: Autopilot did not respond.")
            return False

        waypoint_count = msg.count
        print(f"{self._LOG_PREFIX} Flight controller reported {waypoint_count} mission items.")

        # ArduCopter treats Waypoint 0 as the "Home Position".
        # A valid flight mission requires at least 1 destination waypoint (Total count > 1).
        if waypoint_count <= 1:
            print(f"{self._LOG_PREFIX} Mission rejected: Waypoint list is empty.")
            return False

        # 4. Waypoints exist! Switch flight mode to AUTO to start execution
        print(f"{self._LOG_PREFIX} Waypoints confirmed. Transitioning to AUTO mode...")
