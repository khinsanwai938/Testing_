...
import numpy as np
try:
    import pythoncom  # Required on Windows to stop COM threading crashes
except ImportError:
    pythoncom = None
...
def run_central_pipeline():
    ...
    while True:
        try:
            ...
            # ── Ground commands (allowed before takeoff) ──────────────
            ...
            elif intent == "takeoff" or intent == "go_up":
                value = brain.extract_number(captured_text)
                target_alt = float(value) if value else 5.0
                
                # 1. Pymavlink Native Armed Verification Checks
                if not drone._is_armed():
                    print("[DRIVER] CRITICAL: Takeoff/Ascent rejected! Motors must be armed first.")
                    voice_reply("Command rejected. The propulsion motors are currently disarmed.")
                    log_event(captured_text, intent, confidence, "TAKEOFF_REJECTED_DISARMED")
                    continue
                    
                # 2. Pymavlink Native Ground-Truth Altitude Fetch
                current_altitude = drone._get_altitude()
                if current_altitude is None:
                    current_altitude = 0.0  # Safe fallback if packet hasn't arrived yet
                ...
                # CASE B: Drone is already flying -> Cumulative climb change (10m + 10m = 20m)
                else:
                    new_cumulative_target = current_altitude + target_alt
                    print(f"[DRIVER] Airborne state detected ({current_altitude:.2f}m).")
                    print(f"[DRIVER] Adjusting altitude: Adding {target_alt}m -> New Target: {new_cumulative_target:.2f}m")
                    
                    voice_reply(f"Ascending an additional {int(target_alt)} meters.")
                    
                    if drone.change_altitude_absolute(new_cumulative_target):
                        log_event(captured_text, intent, confidence, f"ALTITUDE_INCREASE_TO_{new_cumulative_target}")
                    else:
